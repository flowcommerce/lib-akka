...
trait Scheduler extends Timers {
  ...
  /** Helper to schedule a recurring interval based on a configuration parameter.
    *
    * Example:
    *
    * conf/base.conf: io.flow.delta.api.CheckProjects.interval = "300 seconds" io.flow.delta.api.CheckProjects.initial =
    * "3 seconds"
    *
    * conf/api/actors/... scheduleRecurring("io.flow.delta.api.CheckProjects", PeriodicActor.Messages.CheckProjects)
    * scheduleRecurring("io.flow.delta.api.CheckProjects", PeriodicActor.Messages.CheckProjects, otherActor)
    *
    * @param sc
    *   The scheduler configuration parameter containing the interval duration between runs. You can also optionally add
  ...
  def scheduleRecurringWithDefault(
    sc: ScheduleConfig,
  ...
  def scheduleRecurring(sc: ScheduleConfig)(f: => Unit)(implicit ec: ExecutionContext): Cancellable = {
